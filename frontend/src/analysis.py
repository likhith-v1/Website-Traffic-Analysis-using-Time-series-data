...
def decompose(series: pd.Series, period: int = 7):
    ...
    Uses ``Series.ffill()`` rather than the deprecated ``fillna(method='ffill')``.
    """
...
def plot_seasonal_subseries(series: pd.Series) -> None:
    ...
    axes[1].bar(
        [months[month - 1] for month in monthly_average.index],
...
def run(
    ...
    # Persist summary so the dashboard can read it if needed.
    summary_path = PRECOMP_DIR / f"{article.replace('/', '_')}_analysis.json"
    summary_path.write_text(json.dumps(summary, indent=2, default=str))
    print(f"Saved analysis summary -> {summary_path.name}")
